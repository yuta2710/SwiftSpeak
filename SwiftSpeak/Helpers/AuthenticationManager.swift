...
class AuthenticationManager: ObservableObject {
    ...
    func signInWithGoogle(callback: @escaping (Bool) -> Void) async -> Bool {
        ...
        do {
            let googleSignInResult: GIDSignInResult = try await GIDSignIn.sharedInstance.signIn(withPresenting: rootViewController)
            ...
            let authComplete = try await Auth.auth().signIn(with: credentialConfig)
            ...
            let isUserExist = try await isUserExistInServer(id: userInfoDetails.uid)
    ...
    func loadCurrentUserProfile() async throws -> Void {
        guard let uId = Auth.auth().currentUser?.uid else {
            return
        }
        
        do {
            let snapshot = try await db.collection("user_profiles").document(uId).getDocument()
            guard let data = snapshot.data() else { return }
            
            DispatchQueue.main.async {
                self.userProfile = UserProfile(id: data["id"] as! String,
